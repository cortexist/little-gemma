...
// BULK weight staging (LG_Q4K_BULK): llama.cpp's load_tiles/vec_dot split. Unpack
// the WHOLE 256-K weight block to shared in ONE phase (all 4 sjp issue their global
// loads together = max memory-level parallelism), THEN a pure-mma compute pass that
// reads only shared. Tests whether bulk-loading hides the weight-DRAM latency (the
// long-scoreboard wall) better than the per-sjp interleaved staging. Costs 4x the
// A-tile shared (sAw holds all 8 sub-blocks) -> ~139KB, still 1 CTA/SM on Orin.
#define SJP_STRIDE (2 * 16 * SA_ROW)              // bytes per (sjp) in sAw_big (2 halves x 16 rows)
template<int COLS>
__global__ static void __launch_bounds__(256, 1)
matmul_q4k_bulk_kernel(float *out, const unsigned char *wbase, int ts,
                       const int8_t *xq, const float2 *xds, int k, int m) {
    extern __shared__ unsigned char sh[];
    int8_t *sB    = (int8_t *)sh;
    float2 *sBxds = (float2 *)(sh + 2 * COLS * SB_COL);
    ...
    int8_t *sAw = sA + warp * 2 * (4 * SJP_STRIDE);          // per warp: 2 tiles x 4 sjp x 2 halves x 16 x SA_ROW
    float acc[2][COLS / 8][4];
    #pragma unroll
    for (int t = 0; t < 2; t++) for (int h = 0; h < COLS / 8; h++) for (int s = 0; s < 4; s++) acc[t][h][s] = 0.0f;
    int rowL0 = r0 + (lane & 15);      if (rowL0 >= m) rowL0 = m - 1;
    int rowL1 = r0 + 16 + (lane & 15); if (rowL1 >= m) rowL1 = m - 1;
    const unsigned char *myrow0 = wbase + (size_t)rowL0 * nbk * ts;
    const unsigned char *myrow1 = wbase + (size_t)rowL1 * nbk * ts;
    mma_stage_b<COLS>(sB, sBxds, 0, xq, xds, k, 0, threadIdx.x);
    asm volatile("cp.async.commit_group;\n");
    ...
    for (int blk = 0; blk < nbk; blk++) {
        ...
        // --- PHASE 1: bulk-unpack the whole 256-K block (all 4 sjp x 2 tiles) ---
        #pragma unroll
        for (int sjp = 0; sjp < 4; sjp++)
            #pragma unroll
            for (int t = 0; t < 2; t++) {
                ...
                const block_q4_K *bp = (const block_q4_K *)(wbase + (size_t)row * nbk * ts) + blk;
                uint4 q4 = *(const uint4 *)(bp->qs + sjp * 32 + hi * 16);
                int8_t *sAt = sAw + t * (4 * SJP_STRIDE) + sjp * SJP_STRIDE;
                uint32_t *lo = (uint32_t *)(sAt + ar * SA_ROW + hi * 16);
                uint32_t *hh = (uint32_t *)(sAt + 16 * SA_ROW + ar * SA_ROW + hi * 16);
                lo[0] = (uint32_t)nib4(q4.x, 0); lo[1] = (uint32_t)nib4(q4.y, 0);
                lo[2] = (uint32_t)nib4(q4.z, 0); lo[3] = (uint32_t)nib4(q4.w, 0);
        ...
        // --- PHASE 2: pure-mma compute, reads only shared ---
        #pragma unroll
        for (int sjp = 0; sjp < 4; sjp++)
            #pragma unroll
            for (int half = 0; half < 2; half++) {
                ...
                float dsc0_1 = __shfl_sync(0xffffffffu, dscL1, gid), dsc1_1 = __shfl_sync(0xffffffffu, dscL1, gid + 8);
                float mnm0_1 = __shfl_sync(0xffffffffu, mnmL1, gid), mnm1_1 = __shfl_sync(0xffffffffu, mnmL1, gid + 8);
                const int8_t *sAh0 = sAw + 0 * (4 * SJP_STRIDE) + sjp * SJP_STRIDE + half * 16 * SA_ROW;
                const int8_t *sAh1 = sAw + 1 * (4 * SJP_STRIDE) + sjp * SJP_STRIDE + half * 16 * SA_ROW;
                uint32_t a0_0 = *(uint32_t *)(sAh0 + gid * SA_ROW + tid * 4), a1_0 = *(uint32_t *)(sAh0 + (gid + 8) * SA_ROW + tid * 4);
                uint32_t a2_0 = *(uint32_t *)(sAh0 + gid * SA_ROW + tid * 4 + 16), a3_0 = *(uint32_t *)(sAh0 + (gid + 8) * SA_ROW + tid * 4 + 16);
                uint32_t a0_1 = *(uint32_t *)(sAh1 + gid * SA_ROW + tid * 4), a1_1 = *(uint32_t *)(sAh1 + (gid + 8) * SA_ROW + tid * 4);
                uint32_t a2_1 = *(uint32_t *)(sAh1 + gid * SA_ROW + tid * 4 + 16), a3_1 = *(uint32_t *)(sAh1 + (gid + 8) * SA_ROW + tid * 4 + 16);
...
// PIPE (LG_Q4K_PIPE): lean + software-pipelined weight prefetch — the causal test on
// the long_scoreboard residual. Issue the NEXT sub-block-pair's weight global loads
// (into registers) BEFORE unpacking+computing the current one, so the ~400-cycle
// load latency overlaps the mma work (independent of the in-flight load). If long_sb
// drops and SM rises, the latency stall was our schedulable ILP; if regs spill and
// it doesn't, it's register-bound. +16 regs (qc0/qc1/qn0/qn1 uint4) on top of the 64 acc.
template<int COLS>
__global__ static void __launch_bounds__(256, 1)
matmul_q4k_pipe_kernel(float *out, const unsigned char *wbase, int ts,
                       const int8_t *xq, const float2 *xds, int k, int m) {
    extern __shared__ unsigned char sh[];
    int8_t *sB    = (int8_t *)sh;
    float2 *sBxds = (float2 *)(sh + 2 * COLS * SB_COL);
    int8_t *sA    = (int8_t *)(sh + 2 * COLS * SB_COL + 2 * COLS * SBX * 8);
    const int warp = threadIdx.x >> 5, lane = threadIdx.x & 31, wpc = blockDim.x >> 5;
    float2 *sSc = (float2 *)(sA + (size_t)wpc * 2 * 2 * 16 * SA_ROW);
    const int gid = lane >> 2, tid = lane & 3, nbk = k / 256;
    xq  += (size_t)blockIdx.y * COLS * k;
    xds += (size_t)blockIdx.y * COLS * (k / 32);
    out += (size_t)blockIdx.y * COLS * m;
    ...
    int row0 = r0 + ar;      if (row0 >= m) row0 = m - 1;       // tile 0 weight row for this lane
    int row1 = r0 + 16 + ar; if (row1 >= m) row1 = m - 1;       // tile 1
    mma_stage_b<COLS>(sB, sBxds, 0, xq, xds, k, 0, threadIdx.x);
    asm volatile("cp.async.commit_group;\n");
    ...
    for (int blk = 0; blk < nbk; blk++) {
        ...
        const int8_t *sBb = sB + buf * COLS * SB_COL;
        const float2 *sBxd = sBxds + buf * COLS * SBX;
        const uint8_t *qs0 = ((const block_q4_K *)(wbase + (size_t)row0 * nbk * ts) + blk)->qs + hi * 16;
        const uint8_t *qs1 = ((const block_q4_K *)(wbase + (size_t)row1 * nbk * ts) + blk)->qs + hi * 16;

        {   // scale staging (same as lean)
            int tl = lane >> 4, row16 = lane & 15;
            int grow = r0 + tl * 16 + row16; if (grow >= m) grow = m - 1;
            const block_q4_K *hp = (const block_q4_K *)(wbase + (size_t)grow * nbk * ts) + blk;
            const uint32_t *sc = (const uint32_t *)hp->scales;
        ...
        uint4 qc0 = *(const uint4 *)(qs0), qc1 = *(const uint4 *)(qs1);   // prologue: sjp=0 weights
        #pragma unroll
        for (int sjp = 0; sjp < 4; sjp++) {
            uint4 qn0, qn1;
            if (sjp + 1 < 4) {                                            // PREFETCH sjp+1 (overlaps compute below)
                qn0 = *(const uint4 *)(qs0 + (sjp + 1) * 32);
                qn1 = *(const uint4 *)(qs1 + (sjp + 1) * 32);
            }
            {   // unpack CURRENT (qc) into shared
                int8_t *sAt0 = sAw + 0 * 2 * 16 * SA_ROW;
                int8_t *sAt1 = sAw + 1 * 2 * 16 * SA_ROW;
                uint32_t *lo0 = (uint32_t *)(sAt0 + ar * SA_ROW + hi * 16), *hh0 = (uint32_t *)(sAt0 + 16 * SA_ROW + ar * SA_ROW + hi * 16);
                uint32_t *lo1 = (uint32_t *)(sAt1 + ar * SA_ROW + hi * 16), *hh1 = (uint32_t *)(sAt1 + 16 * SA_ROW + ar * SA_ROW + hi * 16);
                lo0[0] = (uint32_t)nib4(qc0.x, 0); lo0[1] = (uint32_t)nib4(qc0.y, 0); lo0[2] = (uint32_t)nib4(qc0.z, 0); lo0[3] = (uint32_t)nib4(qc0.w, 0);
                hh0[0] = (uint32_t)nib4(qc0.x, 1); hh0[1] = (uint32_t)nib4(qc0.y, 1); hh0[2] = (uint32_t)nib4(qc0.z, 1); hh0[3] = (uint32_t)nib4(qc0.w, 1);
                lo1[0] = (uint32_t)nib4(qc1.x, 0); lo1[1] = (uint32_t)nib4(qc1.y, 0); lo1[2] = (uint32_t)nib4(qc1.z, 0); lo1[3] = (uint32_t)nib4(qc1.w, 0);
                hh1[0] = (uint32_t)nib4(qc1.x, 1); hh1[1] = (uint32_t)nib4(qc1.y, 1); hh1[2] = (uint32_t)nib4(qc1.z, 1); hh1[3] = (uint32_t)nib4(qc1.w, 1);
            }
            __syncwarp();
            #pragma unroll
            for (int half = 0; half < 2; half++) {
            ...
            __syncwarp();
            qc0 = qn0; qc1 = qn1;                                         // advance the pipeline
        }
    }
    #pragma unroll
    for (int t = 0; t < 2; t++)
...
// PIPE2 (LG_Q4K_PIPE2): 2-deep, cross-block weight prefetch. Treat all 4*nbk
// sub-block-pairs as one continuous stream indexed by g; keep qc(=g)/qn(=g+1) live
// and prefetch g+2 each step. When g+2 crosses a block boundary it loads from the
// next block's weights (base + (blk+1)*ts) -> NO per-block synchronous prologue
// (the long_sb bubble pipe still had at each block's sjp=0). +8 regs vs pipe.
template<int COLS>
__global__ static void __launch_bounds__(256, 1)
matmul_q4k_pipe2_kernel(float *out, const unsigned char *wbase, int ts,
                        const int8_t *xq, const float2 *xds, int k, int m) {
    extern __shared__ unsigned char sh[];
    int8_t *sB    = (int8_t *)sh;
    float2 *sBxds = (float2 *)(sh + 2 * COLS * SB_COL);
    ...
    for (int t = 0; t < 2; t++) for (int h = 0; h < COLS / 8; h++) for (int s = 0; s < 4; s++) acc[t][h][s] = 0.0f;
    const int ar = lane & 15, hi = lane >> 4;
    int row0 = r0 + ar;      if (row0 >= m) row0 = m - 1;
    int row1 = r0 + 16 + ar; if (row1 >= m) row1 = m - 1;
    // block-0 qs bases; block b's qs = base + b*ts, sub-block-pair ps adds ps*32
    const uint8_t *base0 = ((const block_q4_K *)(wbase + (size_t)row0 * nbk * ts))->qs + hi * 16;
    const uint8_t *base1 = ((const block_q4_K *)(wbase + (size_t)row1 * nbk * ts))->qs + hi * 16;
    const int G = nbk * 4;
    #define PIPE2_Q(T, G) (*(const uint4 *)(((T) ? base1 : base0) + (size_t)((G) >> 2) * ts + ((G) & 3) * 32))
    uint4 qc0 = PIPE2_Q(0, 0), qc1 = PIPE2_Q(1, 0);          // g=0
    uint4 qn0 = (G > 1) ? PIPE2_Q(0, 1) : qc0, qn1 = (G > 1) ? PIPE2_Q(1, 1) : qc1;   // g=1
    mma_stage_b<COLS>(sB, sBxds, 0, xq, xds, k, 0, threadIdx.x);
    asm volatile("cp.async.commit_group;\n");
    ...
    for (int blk = 0; blk < nbk; blk++) {
        ...
        for (int sjp = 0; sjp < 4; sjp++) {
            int g = blk * 4 + sjp;
            uint4 qnn0 = qc0, qnn1 = qc1;
            if (g + 2 < G) { qnn0 = PIPE2_Q(0, g + 2); qnn1 = PIPE2_Q(1, g + 2); }   // prefetch g+2 (wraps to next block)
            {   // unpack current (qc) into shared
                int8_t *sAt0 = sAw + 0 * 2 * 16 * SA_ROW, *sAt1 = sAw + 1 * 2 * 16 * SA_ROW;
                uint32_t *lo0 = (uint32_t *)(sAt0 + ar * SA_ROW + hi * 16), *hh0 = (uint32_t *)(sAt0 + 16 * SA_ROW + ar * SA_ROW + hi * 16);
                uint32_t *lo1 = (uint32_t *)(sAt1 + ar * SA_ROW + hi * 16), *hh1 = (uint32_t *)(sAt1 + 16 * SA_ROW + ar * SA_ROW + hi * 16);
                lo0[0] = (uint32_t)nib4(qc0.x, 0); lo0[1] = (uint32_t)nib4(qc0.y, 0); lo0[2] = (uint32_t)nib4(qc0.z, 0); lo0[3] = (uint32_t)nib4(qc0.w, 0);
                hh0[0] = (uint32_t)nib4(qc0.x, 1); hh0[1] = (uint32_t)nib4(qc0.y, 1); hh0[2] = (uint32_t)nib4(qc0.z, 1); hh0[3] = (uint32_t)nib4(qc0.w, 1);
            ...
            __syncwarp();
            qc0 = qn0; qc1 = qn1; qn0 = qnn0; qn1 = qnn1;        // advance the 2-deep pipeline
        }
    }
    #undef PIPE2_Q
    #pragma unroll
...
// PIPE3 (LG_Q4K_PIPE3): 3-deep prefetch (g+3) — keep qc/qn/qnn live, prefetch qnnn.
// +8 regs over pipe2 (~252, right at the 255 spill cliff). The "test until it fails"
// probe: does one more iteration of latency hiding drop long_sb further, or does the
// register file spill and regress? Byte-identical either way.
template<int COLS>
__global__ static void __launch_bounds__(256, 1)
matmul_q4k_pipe3_kernel(float *out, const unsigned char *wbase, int ts,
                        const int8_t *xq, const float2 *xds, int k, int m) {
    extern __shared__ unsigned char sh[];
    int8_t *sB    = (int8_t *)sh;
    float2 *sBxds = (float2 *)(sh + 2 * COLS * SB_COL);
    ...
    const uint8_t *base1 = ((const block_q4_K *)(wbase + (size_t)row1 * nbk * ts))->qs + hi * 16;
    const int G = nbk * 4;
    #define PIPE3_Q(T, G) (*(const uint4 *)(((T) ? base1 : base0) + (size_t)((G) >> 2) * ts + ((G) & 3) * 32))
    uint4 qc0 = PIPE3_Q(0, 0), qc1 = PIPE3_Q(1, 0);
    uint4 qn0 = (G > 1) ? PIPE3_Q(0, 1) : qc0, qn1 = (G > 1) ? PIPE3_Q(1, 1) : qc1;
    uint4 qm0 = (G > 2) ? PIPE3_Q(0, 2) : qn0, qm1 = (G > 2) ? PIPE3_Q(1, 2) : qn1;   // g=2 (qnn)
    mma_stage_b<COLS>(sB, sBxds, 0, xq, xds, k, 0, threadIdx.x);
    asm volatile("cp.async.commit_group;\n");
    ...
    for (int blk = 0; blk < nbk; blk++) {
        ...
        for (int sjp = 0; sjp < 4; sjp++) {
            int g = blk * 4 + sjp;
            uint4 qnnn0 = qm0, qnnn1 = qm1;
            if (g + 3 < G) { qnnn0 = PIPE3_Q(0, g + 3); qnnn1 = PIPE3_Q(1, g + 3); }   // prefetch g+3
            {   // unpack current (qc)
                int8_t *sAt0 = sAw + 0 * 2 * 16 * SA_ROW, *sAt1 = sAw + 1 * 2 * 16 * SA_ROW;
                uint32_t *lo0 = (uint32_t *)(sAt0 + ar * SA_ROW + hi * 16), *hh0 = (uint32_t *)(sAt0 + 16 * SA_ROW + ar * SA_ROW + hi * 16);
                uint32_t *lo1 = (uint32_t *)(sAt1 + ar * SA_ROW + hi * 16), *hh1 = (uint32_t *)(sAt1 + 16 * SA_ROW + ar * SA_ROW + hi * 16);
            ...
            __syncwarp();
            qc0 = qn0; qc1 = qn1; qn0 = qm0; qn1 = qm1; qm0 = qnnn0; qm1 = qnnn1;   // advance 3-deep
        }
    }
    #undef PIPE3_Q
    #pragma unroll
...
// CPW (LG_Q4K_CPW): cp.async the whole block's RAW weights global->shared (sRaw),
// DOUBLE-buffered at block granularity, sharing B's commit/wait cycle (both per-block
// => one group, clean). Frees the qc/qn/qnn registers of pipe2 AND hides the weight-
// DRAM latency via the async-copy unit (overlaps the previous block's compute), so
// it can hide arbitrarily deep without register pressure. sRaw per warp = 2 tiles x
// 4 sjp x 32(ar*2+hi) x 16 B = 4 KB; x2 buffers x wpc = 64 KB (+ sAw 24 KB fits 1 CTA).
#define CPW_WARP (2 * 4 * 32 * 16)                 // bytes of raw qs per warp per buffer
template<int COLS>
__device__ __forceinline__ void cpw_stage(int8_t *sRawBuf, const unsigned char *wbase,
                                          int row0, int row1, int blk, int nbk, int ts, int warp) {
    const int lane = threadIdx.x & 31, ar = lane & 15, hi = lane >> 4;
    int8_t *base = sRawBuf + (size_t)warp * CPW_WARP;
    const block_q4_K *bp0 = (const block_q4_K *)(wbase + (size_t)row0 * nbk * ts) + blk;
    const block_q4_K *bp1 = (const block_q4_K *)(wbase + (size_t)row1 * nbk * ts) + blk;
    #pragma unroll
    for (int sjp = 0; sjp < 4; sjp++) {
        const uint8_t *s0 = bp0->qs + sjp * 32 + hi * 16;
        const uint8_t *s1 = bp1->qs + sjp * 32 + hi * 16;
        unsigned d0 = (unsigned)__cvta_generic_to_shared(base + 0 * (4 * 32 * 16) + sjp * (32 * 16) + (ar * 2 + hi) * 16);
        unsigned d1 = (unsigned)__cvta_generic_to_shared(base + 1 * (4 * 32 * 16) + sjp * (32 * 16) + (ar * 2 + hi) * 16);
        asm volatile("cp.async.cg.shared.global [%0], [%1], 16;\n" :: "r"(d0), "l"(s0));
        asm volatile("cp.async.cg.shared.global [%0], [%1], 16;\n" :: "r"(d1), "l"(s1));
    }
}
template<int COLS>
__global__ static void __launch_bounds__(256, 1)
matmul_q4k_cpw_kernel(float *out, const unsigned char *wbase, int ts,
                      const int8_t *xq, const float2 *xds, int k, int m) {
    extern __shared__ unsigned char sh[];
    int8_t *sB    = (int8_t *)sh;
    float2 *sBxds = (float2 *)(sh + 2 * COLS * SB_COL);
    ...
    const int warp = threadIdx.x >> 5, lane = threadIdx.x & 31, wpc = blockDim.x >> 5;
    float2 *sSc  = (float2 *)(sA + (size_t)wpc * 2 * 2 * 16 * SA_ROW);
    int8_t *sRaw = (int8_t *)(sSc + (size_t)wpc * LEAN_SC);            // [2 bufs][wpc][CPW_WARP]
    const int gid = lane >> 2, tid = lane & 3, nbk = k / 256;
    xq  += (size_t)blockIdx.y * COLS * k;
    xds += (size_t)blockIdx.y * COLS * (k / 32);
    out += (size_t)blockIdx.y * COLS * m;
    ...
    int row1 = r0 + 16 + ar; if (row1 >= m) row1 = m - 1;
    mma_stage_b<COLS>(sB, sBxds, 0, xq, xds, k, 0, threadIdx.x);
    cpw_stage<COLS>(sRaw + 0 * (size_t)wpc * CPW_WARP, wbase, row0, row1, 0, nbk, ts, warp);   // block 0 weights
    asm volatile("cp.async.commit_group;\n");
    ...
    for (int blk = 0; blk < nbk; blk++) {
        ...
        if (blk + 1 < nbk) {
            mma_stage_b<COLS>(sB, sBxds, buf ^ 1, xq, xds, k, blk + 1, threadIdx.x);
            cpw_stage<COLS>(sRaw + (size_t)(buf ^ 1) * wpc * CPW_WARP, wbase, row0, row1, blk + 1, nbk, ts, warp);
            asm volatile("cp.async.commit_group;\n");
        }
        const int8_t *sBb = sB + buf * COLS * SB_COL;
        const float2 *sBxd = sBxds + buf * COLS * SBX;
        const int8_t *rawW = sRaw + (size_t)buf * wpc * CPW_WARP + (size_t)warp * CPW_WARP;
        {   // scale staging
            int tl = lane >> 4, row16 = lane & 15;
            int grow = r0 + tl * 16 + row16; if (grow >= m) grow = m - 1;
            const block_q4_K *hp = (const block_q4_K *)(wbase + (size_t)grow * nbk * ts) + blk;
        ...
        #pragma unroll
        for (int sjp = 0; sjp < 4; sjp++) {
            {   // unpack current sjp's raw (from sRaw, cp.async'd) -> sAw
                uint4 qc0 = *(const uint4 *)(rawW + 0 * (4 * 32 * 16) + sjp * (32 * 16) + (ar * 2 + hi) * 16);
                uint4 qc1 = *(const uint4 *)(rawW + 1 * (4 * 32 * 16) + sjp * (32 * 16) + (ar * 2 + hi) * 16);
                int8_t *sAt0 = sAw + 0 * 2 * 16 * SA_ROW, *sAt1 = sAw + 1 * 2 * 16 * SA_ROW;
                uint32_t *lo0 = (uint32_t *)(sAt0 + ar * SA_ROW + hi * 16), *hh0 = (uint32_t *)(sAt0 + 16 * SA_ROW + ar * SA_ROW + hi * 16);
                uint32_t *lo1 = (uint32_t *)(sAt1 + ar * SA_ROW + hi * 16), *hh1 = (uint32_t *)(sAt1 + 16 * SA_ROW + ar * SA_ROW + hi * 16);
                lo0[0] = (uint32_t)nib4(qc0.x, 0); lo0[1] = (uint32_t)nib4(qc0.y, 0); lo0[2] = (uint32_t)nib4(qc0.z, 0); lo0[3] = (uint32_t)nib4(qc0.w, 0);
...
// Bulk-staging launcher (LG_Q4K_BULK): sAw holds the whole 256-K block (4x), ~139KB.
template<int COLS>
static void launch_q4k_bulk(float *d_out, const unsigned char *w, int ts, const int8_t *xq, const float2 *xds, int k, int m, int sms, int ncol) {
    int wpc = 8;
    while (wpc > 1 && (long)((m + 32 * wpc - 1) / (32 * wpc)) * ncol < 2 * sms) wpc >>= 1;
    size_t shm = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)wpc * 2 * (4 * SJP_STRIDE);
    static int carve = 0;
    if (!carve) {
        size_t maxshm = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)8 * 2 * (4 * SJP_STRIDE);
        if (maxshm > 48 * 1024) cudaFuncSetAttribute(matmul_q4k_bulk_kernel<COLS>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)maxshm);
        carve = 1;
    }
    int blocks = (m + 32 * wpc - 1) / (32 * wpc);
...
// Pipe launcher (LG_Q4K_PIPE): lean + software-pipelined weight prefetch. Same shared as lean.
template<int COLS>
static void launch_q4k_pipe(float *d_out, const unsigned char *w, int ts, const int8_t *xq, const float2 *xds, int k, int m, int sms, int ncol) {
    int wpc = 8;
    while (wpc > 1 && (long)((m + 32 * wpc - 1) / (32 * wpc)) * ncol < 2 * sms) wpc >>= 1;
    size_t shm = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)wpc * 2 * 2 * 16 * SA_ROW + (size_t)wpc * LEAN_SC * sizeof(float2);
    static int carve = 0;
    if (!carve) {
        size_t maxshm = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)8 * 2 * 2 * 16 * SA_ROW + (size_t)8 * LEAN_SC * sizeof(float2);
        if (maxshm > 48 * 1024) cudaFuncSetAttribute(matmul_q4k_pipe_kernel<COLS>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)maxshm);
        carve = 1;
    }
    int blocks = (m + 32 * wpc - 1) / (32 * wpc);
    matmul_q4k_pipe_kernel<COLS><<<dim3(blocks, ncol), 32 * wpc, shm, g_launch>>>(d_out, w, ts, xq, xds, k, m);
}

// Pipe2 launcher (LG_Q4K_PIPE2): 2-deep cross-block prefetch. Same shared as lean/pipe.
template<int COLS>
static void launch_q4k_pipe2(float *d_out, const unsigned char *w, int ts, const int8_t *xq, const float2 *xds, int k, int m, int sms, int ncol) {
    int wpc = 8;
    while (wpc > 1 && (long)((m + 32 * wpc - 1) / (32 * wpc)) * ncol < 2 * sms) wpc >>= 1;
    size_t shm = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)wpc * 2 * 2 * 16 * SA_ROW + (size_t)wpc * LEAN_SC * sizeof(float2);
    static int carve = 0;
    if (!carve) {
        size_t maxshm = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)8 * 2 * 2 * 16 * SA_ROW + (size_t)8 * LEAN_SC * sizeof(float2);
        if (maxshm > 48 * 1024) cudaFuncSetAttribute(matmul_q4k_pipe2_kernel<COLS>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)maxshm);
        carve = 1;
    }
    int blocks = (m + 32 * wpc - 1) / (32 * wpc);
    matmul_q4k_pipe2_kernel<COLS><<<dim3(blocks, ncol), 32 * wpc, shm, g_launch>>>(d_out, w, ts, xq, xds, k, m);
}

// Pipe3 launcher (LG_Q4K_PIPE3): 3-deep prefetch. Same shared as lean/pipe/pipe2.
template<int COLS>
static void launch_q4k_pipe3(float *d_out, const unsigned char *w, int ts, const int8_t *xq, const float2 *xds, int k, int m, int sms, int ncol) {
    int wpc = 8;
    while (wpc > 1 && (long)((m + 32 * wpc - 1) / (32 * wpc)) * ncol < 2 * sms) wpc >>= 1;
    size_t shm = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)wpc * 2 * 2 * 16 * SA_ROW + (size_t)wpc * LEAN_SC * sizeof(float2);
    static int carve = 0;
    if (!carve) {
        size_t maxshm = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)8 * 2 * 2 * 16 * SA_ROW + (size_t)8 * LEAN_SC * sizeof(float2);
        if (maxshm > 48 * 1024) cudaFuncSetAttribute(matmul_q4k_pipe3_kernel<COLS>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)maxshm);
        carve = 1;
    }
    int blocks = (m + 32 * wpc - 1) / (32 * wpc);
    matmul_q4k_pipe3_kernel<COLS><<<dim3(blocks, ncol), 32 * wpc, shm, g_launch>>>(d_out, w, ts, xq, xds, k, m);
}

// CPW launcher (LG_Q4K_CPW): + double-buffered cp.async raw-weight buffer (64 KB).
template<int COLS>
static void launch_q4k_cpw(float *d_out, const unsigned char *w, int ts, const int8_t *xq, const float2 *xds, int k, int m, int sms, int ncol) {
    int wpc = 8;
    while (wpc > 1 && (long)((m + 32 * wpc - 1) / (32 * wpc)) * ncol < 2 * sms) wpc >>= 1;
    size_t base = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)wpc * 2 * 2 * 16 * SA_ROW + (size_t)wpc * LEAN_SC * sizeof(float2);
    size_t shm = base + (size_t)2 * wpc * CPW_WARP;
    static int carve = 0;
    if (!carve) {
        size_t mbase = 2 * COLS * SB_COL + 2 * COLS * SBX * sizeof(float2) + (size_t)8 * 2 * 2 * 16 * SA_ROW + (size_t)8 * LEAN_SC * sizeof(float2);
        size_t maxshm = mbase + (size_t)2 * 8 * CPW_WARP;
        if (maxshm > 48 * 1024) cudaFuncSetAttribute(matmul_q4k_cpw_kernel<COLS>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)maxshm);
        carve = 1;
    }
    int blocks = (m + 32 * wpc - 1) / (32 * wpc);
    matmul_q4k_cpw_kernel<COLS><<<dim3(blocks, ncol), 32 * wpc, shm, g_launch>>>(d_out, w, ts, xq, xds, k, m);
}
...
static void matmul_q_n(float *d_out, const struct gguf_tensor *t, const float *d_x, int k, int m) {
    ...
    static int pipe = -1;                                    // lean + software-pipelined weight prefetch
    if (pipe < 0) pipe = getenv("LG_Q4K_PIPE") != NULL;
    static int pipe2 = -1;                                   // 2-deep cross-block weight prefetch
    if (pipe2 < 0) pipe2 = getenv("LG_Q4K_PIPE2") != NULL;
    static int cpw = -1;                                     // cp.async double-buffered raw-weight staging
    if (cpw < 0) cpw = getenv("LG_Q4K_CPW") != NULL;
    static int pipe3 = -1;                                   // 3-deep register prefetch (g+3)
    if (pipe3 < 0) pipe3 = getenv("LG_Q4K_PIPE3") != NULL;
    static int sms = 0;
    ...
    if (t->type == GGML_TYPE_Q4_K && PREFILL_B % 32 == 0 && !no_mma) {
        ...
        if (!port && !tiled) {
            // pipe3 (LG_Q4K_PIPE3): 3-deep register prefetch (g+3).
            if (pipe3 && g_pf_cols >= 64 && g_pf_cols % 64 == 0) {
                launch_q4k_pipe3<64>(d_out, w, ts, g_xq, g_xds, k, m, sms, g_pf_cols / 64);
                return;
            }
            // cpw (LG_Q4K_CPW): cp.async double-buffered raw-weight staging (frees regs, hides long_sb).
            if (cpw && g_pf_cols >= 64 && g_pf_cols % 64 == 0) {
                launch_q4k_cpw<64>(d_out, w, ts, g_xq, g_xds, k, m, sms, g_pf_cols / 64);
                return;
            }
            // pipe2 (LG_Q4K_PIPE2): 2-deep cross-block weight prefetch.
            if (pipe2 && g_pf_cols >= 64 && g_pf_cols % 64 == 0) {
                launch_q4k_pipe2<64>(d_out, w, ts, g_xq, g_xds, k, m, sms, g_pf_cols / 64);
                return;
            }
            // pipe (LG_Q4K_PIPE): lean + software-pipelined weight prefetch (hide long_sb).
            if (pipe && g_pf_cols >= 64 && g_pf_cols % 64 == 0) {
                launch_q4k_pipe<64>(d_out, w, ts, g_xq, g_xds, k, m, sms, g_pf_cols / 64);
                return;
            }
