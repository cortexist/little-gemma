...
\begin{abstract}
A voice assistant is fluent when the reply begins inside the human turn-taking window (first audio well under a second), and cohesive when a full large language model produces the reply rather than an intent router. Production systems today usually get one property or the other: datacenter ASR→LLM→TTS cascades median 1.4–1.7 s to first audio, while purpose-trained speech-to-speech models such as Moshi beat that window but forfeit the generality of a text LLM. We show both are achievable at once, entirely on a 20 W, credit-card-sized edge board, using unmodified state-of-the-art open-weight models.

Our pipeline pairs streaming speech recognition (whisper.cpp) with a Gemma 4 model (2B–12B) on a purpose-built 6,000-line C/CUDA runner and the piper speech synthesizer — reaches 0.65 s from end of speech to first audio on a Jetson Orin NX 16GB for a 30-second spoken instruction. The full stack holds 4.2 GB of unreclaimable memory (it fits an 8 GB Jetson Nano) and peaks near 20 W.

Four techniques carry the result, none of which modify the models: 
(1) prefill under speech, so most of the prompt is already processed when the user finishes; 
(2)  The model as its own clause splitter, creating early TTS flush points that stock models otherwise omit; 
(3) a streaming vocoder obtained by piper's graph surgery on stock piper voices, so first audio is constant-time in clause length; and 
(4) multi-token prediction speculative decoding, characterized as content-dependent for latency and beneficial for energy. 

With the family’s repaired native audio encoder, the same engine reaches 0.46–0.54 s first audio without whisper as a hard dependency. A cross-silicon comparison against Moshi shows that the remaining gap to speech-native models is structural (~0.2 s for one clause plus one vocoder call), and that a datacenter GPU buys only ~1.7× over the 20 W board: in the conversational regime the pipeline is floor-bound, not compute-bound.
\end{abstract}
...
\begin{itemize}
	\item voicecat + whisper.cpp (CUDA base.en): energy VAD to open an utterance, streaming transcription with LocalAgreement-2 commits, timestamp-based window trimming, barge-in (one byte stops the reply mid-stream), transcript-verdict endpointing (consecutive ASR passes with zero words are the ear's own ``nothing is being said'' --- the turn closes even under sustained music, which pins a pure energy VAD open), and ambient-sound captions (whisper's bracketed non-speech tags, deduplicated, ride the turn close so the model can truthfully answer about what it heard). Committed words stream into the runner as typed text frames while the turn is still open.
...
\begin{itemize}
	\item TTFT: last user input $\rightarrow$ first reply token (server work becomes visible).
	\item TTFS / time-to-first-speakable: $\rightarrow$ first complete speakable unit (sentence, or clause once Section \ref{sec:clause} applies) --- the unit the TTS can start on.
	\item TTFB (first audio): $\rightarrow$ first PCM sample out of the TTS. The number the ear judges. TTFT alone misorders systems: a fast first token followed by a 21-word sentence is slower to speak than a slower first token followed by a 6-word clause.
\end{itemize}
...
\begin{figure}[H]
	...
	\caption{In deferred delivery (a) the 5.10 s prefill starts only when the turn closes; streamed (b), committed words prefill during the speech itself and the turn close leaves only the holdback span --- TTFT 5.10 $\rightarrow$ 0.53 s with byte-identical replies (12B QAT).}
	\label{fig:prefill-under-speech}
\end{figure}

Measured on the Orin (TTFT after last word, deferred $\rightarrow$ paced): 12B 5.10 $\rightarrow$ 0.53 s, E4B 2.04 $\rightarrow$ 0.16 s, E2B 1.15 $\rightarrow$ 0.10 s. The mechanism generalizes beyond dictation: camera and video frames prefill on arrival the same way (media spans, 3.0× video TTFT), which is what makes the same pipeline camera-capable --- and, once the QAT releases repaired the family's native audio encoder, raw audio chunks ride it too (Section \ref{sec:threetiers}, TTFT flat in utterance length with zero engine changes) --- the flexibility argument against purpose-trained speech models made concrete.
...
Effect on the E2B voice loop: first speakable unit 21 $\rightarrow$ 8 words (0.714 $\rightarrow$ 0.549 s), synthesis of that unit 0.49 $\rightarrow$ 0.27 s (VITS synthesis is $\sim$linear in audio length, so shorter first units pay twice), first audio 1.21 $\rightarrow$ 0.82 s. Two honest notes travel with this: style pressure sharpens a small model into confident error (the 8-word answer contains a wrong date; the vague baseline was defensible), and prompt-only is the bootstrap --- the production path is a light finetune for human pause placement, which is often shorter than grammatical clauses.
...
Orin NX 16GB, JetPack 6.2.1, pinned clocks for benchmarks (jetson\_clocks; production ships DVFS --- a reboot resets the pin, and every number here was re-verified against that trap), models: Gemma 4 QAT q4\_0 releases across the family --- 12B-it, E4B-it, E2B-it (Google's quantization-aware checkpoints, unsloth GGUFs), each with its matched MTP head; whisper.cpp CUDA base.en; piper en\_US voices. Cross-silicon: RTX A5000 (Windows) + i7-8086K. All measurements are client-side per Section \ref{sec:metrics}. The E4B and 12B rows in Sections \ref{sec:substrate-eval}, \ref{sec:dictmatrix} and \ref{sec:threetiers} were re-measured 2026-07-19 on the QAT builds (which sped up both stacks --- llama.cpp's q4\_0 decode gains more than ours); the multimodal comparison in Section \ref{sec:substrate-eval} and the 12B/E4B rows of the cross-silicon study (Section \ref{sec:crosssilicon}) predate the switch (Q4\_K\_M builds) and are labeled in place --- the switch only improves our side of those.
...
\begin{figure}[H]
	...
	\caption{The same question, three configurations. The voice-sys prompt cuts the first speakable unit from 21 to 8 words (1.21 $\rightarrow$ 0.82 s); the streaming vocoder (Section \ref{sec:vocoder}) then decouples first PCM from clause length entirely (0.82 $\rightarrow$ 0.65 s). Synthesis continues under playback.}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{l X X X}
		...
		\bottomrule
	\end{tabularx}
	\caption{One runner, one board, three whisper-lane operating points (TTFS + TTS $\approx$ TTFB; 30-second spoken instruction, paced delivery).}
...
observed for text streaming in Section~\ref{sec:prefill}. Table~\ref{tab:native-hearing}
...
\bottomrule
\end{tabular}
\caption{Native-hearing latency (QAT builds, voice prompt, MTP), measured on
warm runs with client-side clocks. The E2B reaches first audio in
$\approx$0.46\,s including speech recognition, the fastest configuration
evaluated.}
...
\textbf{Both lanes ship deliberately, because choosing an ear is choosing a
point on a fusion spectrum.} The axis is how much of the system lives inside
model weights. Moshi fuses everything --- hearing, speaking, turn-taking --- and
is the fastest ($\sim$0.13 s, Section \ref{sec:crosssilicon}) and least
governable: the backbone is frozen (Section 2). The whisper lane fuses
nothing: between ear and brain sits an editable transcript, and the pipeline
can gate, correct, or annotate what the model reads before it reads it. The
native lane sits between whisper and Moshi: hearing fuses into the checkpoint
(one fewer process, 0.46--0.54 s first audio), the brain stays a swappable
text LLM --- but the text gap where a cascade keeps its policy is closed. Each
step toward fusion buys latency by surrendering exactly that surface, and
what the surface is worth stops being abstract the moment the audio gets
complicated. Speech over background music --- a living-room default --- needs
three things, and all three live in the gap. \textit{Endpointing} needs a
mid-utterance content verdict (``the talker stopped; the sound continues''):
the whisper lane has one --- consecutive ASR passes over the open window
returning zero words close the turn even as the music plays on (Section 3) ---
while the native lane's first content judgment is the model's reply itself,
too late to gate a turn, and the cheap substitutes fail measurably (an energy
VAD holds the turn open for the duration of the music; a direction-of-arrival
tracker promotes a loud phone speaker to a talker). \textit{Awareness} needs
a non-speech classifier: whisper's bracketed sound captions ([Music]) ride
the turn close as real detections the model can truthfully report; the native
encoder is speech-only by measurement and confabulates when prompted about
non-speech (Section \ref{sec:limitations}). \textit{Improvement} needs
training freedom: the whisper ear is a 74M-parameter model behind a text
interface --- finetunable for a designed tag vocabulary, noise robustness, or
one room's acoustics with zero risk to the LLM --- while the native ear's
weights are fused into the multimodal checkpoint whose fragilities
Section \ref{sec:limitations} catalogs. Add the 12B's vision-hearing
exclusion (Section \ref{sec:limitations}), and the shipping policy follows:
whisper is the default lane; native ears are the speech-only, vision-free
configuration that buys back most of the speech-native species' latency
while keeping the swappable brain --- and gives up the governable ear. Honest
scope: speech-only (non-speech confabulates); vision in the same session
still degrades hearing (a checkpoint behavior --- the engine delivers both
modalities in one turn and needs no change when a fixed checkpoint lands);
the chunk dial is validated on one clip so far.


\subsection{Memory: does it fit an 8 GB Nano?}
\label{sec:memory}

Measured the honest way for Jetson (anonymous RSS + nvmap; VmHWM counts evictable file pages and MemAvailable under-counts nvmap --- both mislead): little-gemma E2B QAT serve 3.2 GB unreclaimable (nvmap 3.00 + anon 0.11, after mmap'ing the weight blob and skipping the pin for fully-repacked models: 5.5 $\rightarrow$ 3.2 GB), whisper $\sim$0.7 GB, piper 0.31 GB peak $\rightarrow$ $\approx$ 4.2 GB for the full voice stack, leaving $\sim$2.2 GB slack on a 7.4 GB-usable headless Nano, with room for the vision encoder ($\sim$0.7 GB). Caveat, stated plainly: measured on the NX 16GB against an 8 GB budget, not yet on the Nano SKU.

\subsection{Power}

GPU-saturated decode peaks $\sim$23 W (default profile; 40 W MAXN raises clocks but the workload is bandwidth-bound), idle-to-peak on a supply a robot already carries. MTP reduces J/token 2.85 $\rightarrow$ 2.29 (Section \ref{sec:speculative}). Moshi's reference deployment is an L4 (24 GB, $\sim$72 W TDP) datacenter GPU.
...
\begin{table}[H]
	...
	\begin{tabular}{ccccc}
		\toprule
		model  &  1st token & 1st clause & +piper & word $\rightarrow$ audio \\
		\midrule
		12B (Q4\_K\_M) &  0.090& 0.241 & 0.109&0.35 s\\
		E4B (Q4\_K\_M) & 0.065 & 0.132 & 0.108&0.24 s\\
...
(Quantization era is per-row: this study predates the QAT default switch on
the 12B/E4B tiers --- Section \ref{sec:eval} --- while the E2B tier was already
QAT. The switch sped up decode in absolute terms on every tier, so a QAT
re-run --- listed in Section \ref{sec:future} together with streaming TTS on
both sides --- could only tighten our side; the readings below do not depend
on it.)
...
The cleanest ablation of our techniques is another cascade with everything else held equal. HuggingFace's speech-to-speech pipeline (v0.2.10, the June 2026 PyPI release --- their codebase moves quickly, so the pin matters) ran on the same Orin NX against the same Gemma E2B QAT weights (served by an unmodified llama-server on the GPU; faster-whisper and MMS-TTS on CPU --- the same ears/mouth placement we chose, reached there by necessity since PyPI CUDA wheels cannot see the Tegra iGPU). End of speech $\rightarrow$ first reply audio, live-paced 16 kHz socket input, warm medians:
...
\begin{table}[H]
	...
	\begin{tabular}{>{\raggedright\arraybackslash}p{4cm}cc}
		...
		\midrule
		HF speech-to-speech, defaults & $\sim$1.6 s & 0.9--2.2 s \\
...
\begin{figure}[H]
	...
	\caption{TTFT and first audio (measured TTFS + the streaming vocoder's 0.10 s first-PCM constant, Section \ref{sec:vocoder}) as one conversation fills the window. Both rise gently and slightly sub-linearly --- TTFT 0.25 $\rightarrow$ 0.45 s, first audio 0.49 $\rightarrow$ 0.75 s --- and both stay inside the conversational band for the entire session. Plain decode slows 39.6 $\rightarrow$ 28.6 tok/s (attention over the growing KV), still far above the $\sim$3 tok/s real-time-listening floor (Section \ref{sec:discussion}).}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lXX}
		...
		Native ears usable for speech; whisper remains the default lane. \\
		...
		Music/ambient require an external tag --- which the whisper lane supplies (its sound captions ride the turn close, Section 3). \\
...
\begin{thebibliography}{19}
	...
	D.~Mach\'a\v{c}ek, R.~Dabre, and O.~Bojar, ``Turning Whisper into real-time transcription system,'' \emph{arXiv preprint arXiv:2307.14743}, 2023. Presented at IJCNLP-AACL 2023 System Demonstrations. [Online]. Available: \url{https://arxiv.org/abs/2307.14743}
	...
	\bibitem{roy2026personaplex}
	R.~Roy, J.~Raiman, S.-g.~Lee, T.-D.~Ene, R.~Kirby, S.~Kim, J.~Kim, and B.~Catanzaro, ``PersonaPlex: Voice and role control for full duplex conversational speech models,'' \emph{arXiv preprint arXiv:2602.06053}, 2026. [Online]. Available: \url{https://arxiv.org/abs/2602.06053}. NVIDIA's release materials (model card and project page) state the 7B model is powered by the Moshi streaming architecture and the Mimi codec; the Moshi-lineage detail is sourced there, in the style of \citep{kyutai2024moshirepo}.
